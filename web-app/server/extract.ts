...
/** Pure scrape function — can be called directly by aiWorker without Express */
export async function extractUrl(requestedUrl: string): Promise<ExtractResult> {
...
/** Express handler — thin wrapper around extractUrl() */
export async function handleExtract(req: Request, res: Response) {
  const { url: requestedUrl } = req.body;
  if (!requestedUrl) return res.status(400).json({ error: "URL is required" });

  try {
    const result = await extractUrl(requestedUrl);
